...
final class SyncConversions extends Command
{
    /**
    ...
    protected $signature = 'google-ads:sync-conversions
                            {campaign? : The campaign ID to sync conversions for}
                            {--start-date= : Start date for syncing (YYYY-MM-DD)}
                            {--end-date= : End date for syncing (YYYY-MM-DD)}
                            {--all : Sync conversions for all Google Ads campaigns}';
     ...
     * @var string
     */
    protected $description = 'Sync conversions from Google Ads API for campaigns';
    ...
    public function handle(GoogleAdsService $googleAdsService): int
    {
        ...
        $this->info("Syncing conversions for {$campaigns->count()} campaign(s)...");
        ...
        foreach ($campaigns as $campaign) {
            try {
                $synced = $googleAdsService->syncConversions($campaign, $startDate, $endDate);
                $totalSynced += $synced;
                $this->newLine();
                $this->info("✓ Synced {$synced} conversions for campaign: {$campaign->name}");
            } catch (Exception $e) {
                $failed++;
                $this->newLine();
        ...
        $this->table(
            ...
            [
                ...
                ['Total conversions synced', $totalSynced],
